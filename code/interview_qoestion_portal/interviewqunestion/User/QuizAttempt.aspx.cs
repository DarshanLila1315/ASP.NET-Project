    ...
    public partial class QuizAttempt : System.Web.UI.Page
    {
        ...
        protected void btnSubmitTest_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Store user answers for saving later
                List<UserAnswerData> userAnswers = new List<UserAnswerData>();
                    ...
                    if (item.ItemType == ListItemType.Item || item.ItemType == ListItemType.AlternatingItem)
                    {
                        ...
                        if (hfQuestionId != null)
                        {
                            ...
                            if (rows.Length > 0)
                            {
                                ...
                                {
                                    // No answer selected - count as wrong, use empty string
                                    wrongCount++;
                                    selectedAnswer = " "; // Use space instead of empty to avoid DB issues
                                }

                                // Store answer data
                                userAnswers.Add(new UserAnswerData
                                {
                ...
                // Save result to database and get Result_ID
                Dictionary<string, dynamic> parameters = new Dictionary<string, dynamic>();
                parameters["@p_Test_ID"] = testId;
                parameters["@p_User_ID"] = userId;
                ...
                DataTable dtResult = db.ExeSP("sp_InsertTestResult", parameters);
                int resultId = 0;
                
                if (dtResult != null && dtResult.Rows.Count > 0)
                {
                    resultId = Convert.ToInt32(dtResult.Rows[0]["Result_ID"]);
                }
                ...
                if (resultId > 0)
                {
                    foreach (var answer in userAnswers)
                    {
                        try
                        {
                            Dictionary<string, dynamic> answerParams = new Dictionary<string, dynamic>();
                            answerParams["@p_Result_ID"] = resultId;
                            answerParams["@p_Question_ID"] = answer.QuestionId;
                            answerParams["@p_User_Answer"] = answer.UserAnswer;
                            answerParams["@p_Is_Correct"] = answer.IsCorrect;

                            db.ExeSP("sp_Save_User_Answer", answerParams);
                        }
                        catch (Exception ex)
                        {
        ...
        // Helper class to store user answer data
        private class UserAnswerData
        {
