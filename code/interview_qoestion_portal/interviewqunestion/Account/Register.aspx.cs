    ...
    public partial class Register : System.Web.UI.Page
    {
        ...
        public static bool CheckEmailExists(string email)
        {
            ...
            try
            {
                ...
                // Use the same SP as Login for consistency
                Dictionary<string, dynamic> para = new Dictionary<string, dynamic>();
                string trimmedEmail = email.Trim();
                para["@p_Email"] = trimmedEmail;
                
                DataTable dtExists = null;
                try {
                    dtExists = db.ExeSP("sp_GetUserByEmail", para);
                } catch {}
                ...
                // Fallback: Check strictly via query if SP failed or returned nothing (paranoia check)
                // This aligns with Login.aspx.cs fallback to catch admin/legacy cases
                string sql = $"SELECT * FROM User_Master WHERE User_EmailID = '{trimmedEmail}'";
                try {
        ...
        public static object SendVerificationOTP(string email)
        {
            try
            {
                ...
                if (emailSent)
                {
                    ...
                    // DEBUG: Include OTP in response for testing (REMOVE IN PRODUCTION!)
                    return new { success = true, message = "OTP sent successfully", debugOTP = otp };
                }
                else
                {
        ...
        protected void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(pass) || !System.Text.RegularExpressions.Regex.IsMatch(pass, @"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$"))
            {
                ShowTerminalError("Registration Failed: Password does not meet security requirements.");
                return;
            }
